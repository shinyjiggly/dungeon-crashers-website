...
* There are no restrictions on use based on the content of the game.
*
* The following actions are prohibited.
* ・Pretending that you made this
* ・Selling this plugin itself as a resource
* (Bundling with other resources is not prohibited)
* Please include the following in the read me file.
 * しぐれん：[github]:https://github.com/Sigureya/RPGmakerMV
 * 
 */
/*:
* If you have modified it, please indicate this after the name in the form "Modified by (XX)" or similar.
 * @author しぐれん (Modified by lavendersiren)
 * @plugindesc  Personal item caches!
 *
 * @param menuCommand
 * @text command name
 * @desc Command name for having an item on the menu screen
 * @type string
 * @default item got
 * 
 * @param menuCommandPosition
 * @text command position
 * @desc Adjust the location of the item possession command
 * @type select
 * @option coordinate origin of the command
 * @value 0
 * @option above items
 * @value 1
 * @default 0
 * 
 * @param usingWeight
 * @text enable encumberance
 * @desc Defines whether to use the item weight feature.
 ...
 * @param defaultWeight
 * @text default weight
 * @desc default weight of the item
 * if you don't write in the comment box, this will be the fallback value
 * @type number
 * @default 1
 * @parent usingWeight
 * 
 * @param weightText
 * @desc Weight display format (when specifying quantity)
 * @default Weight
 * @parent usingWeight
 * 
 * @param command2
 ...
 * @param command
 * @text Command list
 * @desc This is a list of commands available in the game.
 * You can adjust the sort order here.
 ...
 * @param use
 * @type struct<CommandSetting>
 * @desc Command name to use the item
 * @default {"name":"use","description":""}
 * @parent command
 * 
 * @param swap
 * @type struct<CommandSetting>
 * @desc Command name to swap items
 * @default {"name":"Exchange","description":""}
 * @parent command
 * 
 * @param remove
 * @type struct<CommandSetting>
 * @desc Command name for putting away an item
 * @default {"name":"Put it back","description":""}
 * @parent command
 * 
 * @param add
 * @type struct<CommandSetting>
 * @desc Command name for adding an item
 * @default {"name":"add","description":""}
 * @parent command
 * 
 * @param pass
 * @type struct<CommandSetting>
 * @desc The command name to pass the item to
 * @default {"name":"give","description":""}
 * @parent command
 * 
 * 
 * @param myset
 * @type struct<CommandSetting>
 * @desc "my set" command name
 * @default {"name":"my stuff","description":"You can save the combination of items you own."}
 * @parent command
 * 
 * @param sort
 * @type struct<CommandSetting>
 * @desc Command name for sorting items
 * @default {"name":"sort","description":""}
 * @parent command
 * 
 * @param numberSelectHelp
 * @type string
 * @desc Sets the help text when selecting the quantity
 * @default Use the left and right keys to select the quantity
 * @param sound
 * @text sfx
 * 
 ...
 * @type switch
 * @desc when this thing is ON,
 * put the new item in the party leader's pocket
 * 
 * @param pocketKeepByremoveActor
 * @desc when this thing is ON,
 * Keeps the item when the holding actor leaves the party.
 * @type switch
 * @default 0
 * 
* 
 ...
 * @param maxAmount
 * @type number
 * @desc Cap on a pacticular item amount
 * @default 99
 * @parent pocket
 * 
 * @param pocketSize
 * @type number
 * @desc set the type of item that can go into the pocket...?
  * (I think it might actually be how many slots an inventory can have)
 * @default 6
 * @parent pocket
 * 
 * @param canDuplicate
 * @type boolean
 * @Defines how many items of the same type can be pocketed....?
 * (I think it might actually check if an items can have dupes)
 * @default false
 * 
 * @param MaxColor
 * @type number
 * @desc what color to display when you hit max item capacity
 * (It is specified by the system color number)
 * @default 18
 * @parent pocket
 * 
 * @param NotEnoughColor
 * @type number
 * @desc Sets the color to be used when there are insufficient items when executing My Set.
 * @(not implemented!!)
 * @default 4
 * 
 * 
 * @param usingMyset
 * @type boolean
 * @desc Define whether to use the My Set function.
 * @on use
 * @off don't use
 * @default true
 * @parent myset
 ...
 * @param mysetSize
 * @type number
 * @desc Define the number of My Sets to save
 * @default 8
 * @parent myset
 * 
 * @param mysetFormat
 * @type string
 * @desc This is the default name for My Set.
 * @default My Set 【%1】
 * @parent myset
 * 
 * @param saveMyset
 * @type string
 * @desc This is the command to save your My Set.
 * @default save My Set
 * @parent myset
 * 
 * @param saveMysetHelp
 * @type string
 * @desc Help display when saving a My Set
 * @default select a save location
 * @parent myset
 * 
 * @param loadMyset
 * @type string
 * @desc This is the command used to load a My Set.
 * @default Loading My Sets
 * @parent myset
 * 
 * @param loadMysetHelp
 * @type string
 * @desc Help display when loading a My Set
 * @default select data to load
 * @parent myset
 * 
 * @param renameMyset
 * @type string
 * @desc This is the command name used to change the registered name of a My Set.
 * @default change name of My Set
 * @parent myset
 *  
 * @param developMode
 * @type boolean
 * @desc Development mode flag. Contains dummy data and other settings.
 * @default false
 * 
 * @help
 * Each actor can carry an item.
 * If the switch setting parameter is not specified, it will be treated as false (OFF).
 * <MaxAmount:2>
 * By writing the above on the item side, you can limit the number carried.
 * In this case, you can limit the number of items of that type that can be put in a pocket to 2.
 * <Weight:8>
 * You can set the weight of the item.
 * If not set, the default weight will be specified.
 * 
 * 
 * ■ Checking if you have an item -
 * Checks to see if you have an item are not performed on pockets.
 * If you want to check with an event command, you can execute checkPocket.
 * 
 * Check with a script
 * You can also check by writing the following in a conditional branch (script).
 * $gameParty.isInPocket(item number);
 * To check if a specific actor has it, use the isInPocket() function in Game_Actor.
 * Here's how to call it:
 * actor.isInPocket(item number)
 * $gameActor.actor(actor number).isInPocket(item number)
 * 
 * ■ about the program command -
 * If you want to check items in your pocket using conditional branching,
 * execute one of the following plugin commands.
 * If you change the setting, it will only take effect once for the next conditional branch.
 * ◆Pocket PartyInclude -
 * check the pockets of those in your party
 * ◆Pocket AllInclude
 * checks all of the items across all actors
 * (there was a bug in dragon quest 4 that was caused by this not being a thing
 *  so this is here to prevent that in your game.)
 * 
 * ◆Pocket SaveTemporary
 * stores contents of actor pockets and wipes them to default
 * (this is just like that island challenge in breath of the wild)
 * This data will be recorded in $gameParty as part of your save data.
 * you can only save one temp pocket at a time or else the program will DIE!!
 * 
 * ◆Pocket LoadTemporary
 * restores the pocket data from the savetemporary slot
 * (this also wipes the data in the temp slot)
 * Don't call this unless you have something in the temp slot! the program will die!!
 * 
 * ■ About notes -
 * you can set an actor's default inventory in the actor notebox
 * if you post something like this, the chosen actor when initialized will have these things in their inventory:
 * four of item 1 and three of item 2 (using database ID's)
 * <PocketItem[1]:4>
 * <PocketItem[2]:3>
 * 
 * ■ About potential conflicts -
 * This plugin may conflict with those that redefine Game_Battler.consumeItem
 * Because this isn't using Game_Battler.consumeItem, its using a custom thing instead.
 * if it breaks, contact the main dev on twitter, if it wasn't so many years after the 
 * fall of twitter and the main dev wasn't working on a premium version of this plugin.
 * 
 * ■ method naming conventions -
 * Example:
 * execute**()
 * a function that executes **
 * the part to open the necessary window is in **
 * start**
 * end**
 * in Scene_ItemPocket:
 * this is how you start and end it:
 * window opens with start and closes with end
 * 
 * ver EN 2.0.1（2025/06/12）
 * disabled myset because I'm not using it
 * machine and personal translated a lot of things
 *
 * ver 2.0.0（2018/03/22）
 * touch operation bugfixing
 * Changed the body class syntax from the traditional one to the ES5 one.
 * (might be really really buggy!)
 * ver 2.1.0 (2018/03/28)
 * More help and better UI!
 * changed some plugin parameters (less compatibility)
 *
 * 
 * ver 1.4.0
 * added touch-enabled buttons!
 * 
 * ver 1.3.0 
 * fixed a bug in MA_ItemPocket.addItem()
 * added the item passing function!
 * ver 1.2.0(2017/09.13) added item weight!
 * ver 1.1.0(2017/09/05) added the My Set feature!
 * ver 1.0.0(2017/08/26) added ability to check for items in the conditional branch! (again)
 * (also an added option for dragon quest style stuff) 
 * ver 0.7.5(2017/06/28) you can now specify amounts of items to add or remove, with a grayed out option if you can't
 * ver 0.7.0(2017/06/21) this now works in battle!
 * ver 0.6.3(2017/06/21) slight amount of in-battle support
 * ver 0.6.0(2017/06/20) you can now hold items!
 * ver 0.5.0(2017/06/20) made it public!
 */
/*~struct~PocketItem:
 * @param id
 * @param amount
 ...
 * @param description
 * @type string
 * stuff that goes on top of the screen ^
 */
 ...
 * TODO:
 * 
 * Implement Pocket_Base class and improve functionality.
 * Make Dragon Quest-style possession mode the default standard.
 * 
 * display improvements:
 * (it looks weird while scrolling)
 * 
 * add the equipment in there (maybe stick it in the item thing too?)
 * (maybe just grab the data from the actor?)
 * 
 * Touch improvements
 * passing items between actors via touch controls
 * rotating quantity selection by 90 degrees (???)
 * make pressing cancel unnecessary when switching modes
 * by passing nothing, you can just pull an item from the other side
 * 
 * Process to give from item list (call with specific button)
 * The following functions can be called up with one button:
 * (call destination can be changed)
 * sort
 * clear
 * maxumum replenishment
 * Assign this to XY. Check YES/NO just before using.
 * Ability to set traits on held items in the same way as equipment.
 * Implemented a button for click operation based on ShopNumber
 * Put it away and put it back in with one push of a button
 * 
 * Shop support!
 * Binding for event commands
 * (Increase items, hook items)
 * 
 * Submenu
 * Displayed when the Enter key is pressed
 * ◆Pocket Sync true
 * When you execute an item-related event command,
 * it will also be executed on items in your pocket.
 * The effect is only valid within the event in which this event command was executed.
 * ◆Pocket Sync false
 * Used to cancel SyncON.
 * 
 * The following event commands are applicable:
 * ・Increasing or decreasing items (when decreasing items)
 * ・Conditional branching (checking for a specific item?)
 * ・Variable manipulation (number of items owned)
 * 
 * Recently used item
 * If you have the maximum number of items, the number will be displayed in red.
 * When reducing an item, make it reduce the amount held by the actor in question
 * Do the same as the process of reducing equipment.
 * Run it with a plugin command? Set a temporary flag?
 * <ActorEquipItem:
 *   max =10
 *   
 * >
...
class Scene_ItemPocket extends Scene_ItemBase {
    ...
    createAllWindow() {
		...
		}   //edit: made myset only show up if its active!!
        this.addNameWindows();
...
//I'm registering commands for individual possession.
